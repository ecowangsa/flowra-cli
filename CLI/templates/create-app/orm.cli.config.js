  ...
  return {
    ...
    migrations: {
      ...
      directory: resolveDirectory(
        config.migrations?.directory || 'app/Database/Migrations',
        migrationsDir || process.env.KNEX_MIGRATIONS_DIR
      ),
    },
    seeds: {
      ...config.seeds,
      directory: resolveDirectory(config.seeds?.directory || 'app/Database/Seeds', seedsDir || process.env.KNEX_SEEDS_DIR),
    },
    log: config.log,
    connectionName: key,
  };
